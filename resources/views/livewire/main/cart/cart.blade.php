                        ...
                        <div class="mt-12 lg:grid lg:grid-cols-12 lg:gap-x-12 lg:items-start xl:gap-x-16">
                            ...
                            <dl class="mt-6 space-y-4">
                                ...
                                <div class="border-t border-gray-200 dark:border-zinc-600 pt-4 flex items-center justify-between">
                                    ...
                                    <dd id="gig-amount" class="text-base font-medium text-gray-900 dark:text-white">@money($this->total() + $this->taxes(), settings('currency')->code, true)
                                        <button type="button" data-tooltip-target="approx-usd" class="ltr:ml-2 rtl:mr-2 flex-shrink-0 text-gray-400 hover:text-gray-500">
                                                <svg class="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"> <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5 1 1 0 11-1.731-1A3 3 0 0113 8a3.001 3.001 0 01-2 2.83V11a1 1 0 11-2 0v-1a1 1 0 011-1 1 1 0 100-2zm0 8a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"/> </svg>
                                        </button>
                                        
                                        <div id="approx-usd" role="tooltip" class="inline-block absolute invisible z-10 py-2 px-3 text-sm font-medium text-white bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
                                            $ {{ number_format($totalAmountUSD, 2) }}<br>{{ __('messages.t_currency_conversion_info') }}  .
                                        </div>
                                    </dd>
...
<script>
    (function() {
        const amountElement = document.getElementById('gig-amount');
        const approxUSDElement = document.getElementById('approx-usd');
        const INRtoUSDConversionRate = 0.014; // Default value

        // Function to convert INR to USD
        function convertINRtoUSD(amountINR) {
            return amountINR * INRtoUSDConversionRate;
        }

        // Fetch the exchange rate from your custom proxy
        fetch('https://skillmonde.com/currency-proxy.php')
            .then(response => response.json())
            .then(data => {
                // Extract the exchange rate from the API response
                const exchangeRate = data.rates.INR;

                const totalAmountINR = parseFloat('{{ $this->total() + $this->taxes() }}');
               // const totalAmountUSD = convertINRtoUSD(totalAmountINR);
                
                const message = 'Please note that the final price may fluctuate during the transaction.';

                // Update the DOM with INR and USD amounts
                approxUSDElement.innerHTML = `$ ${(totalAmountINR / exchangeRate).toFixed(2)}<br>${message}`;
            })
            .catch(error => {
