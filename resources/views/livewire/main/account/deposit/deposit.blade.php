                                ...
                                <div class="relative grid grid-cols-3 sm:gap-4 gap-y-4 mt-14" wire:key="deposit-key-select">
                                    ...
                                    @if (settings('paytr')->is_enabled)
                                        <div 
                                            ...
                                            {{-- Deposit fee --}}
                                            <span class="text-green-400 text-xs" style="width: 220px;">{{ settings('paytr')->deposit_fee }}% {{ __('messages.t_fee') }}</span>
                                    ...
                                    @if (settings('razorpay')->is_enabled)
                                        <div 
                                            ...
                                            {{-- Deposit fee --}}
                                            <span class="text-green-400 text-xs" style="width: 220px;">{{ settings('razorpay')->deposit_fee }}% {{ __('messages.t_razorpay_fee') }} </span>
                                                        ...
                                                        <dd class="font-medium text-gray-900 dark:text-gray-200">
                                                            <div class="flex items-center text-sm text-gray-500 dark:text-gray-300">
                                                                @if (settings($selected)->logo)
                                                                    <img src="{{ placeholder_img() }}" data-src="{{ src(settings($selected)->logo) }}" class="lazy max-w-[150px]">
                                                                @endif
...
<script>
    $(document).ready(function() {
        ...
        // Check if the 'add_fund' parameter exists in the URL
        if (urlParams.has('add_fund')) {
            // Get the value of the 'add_fund' parameter
            const depositAmount = urlParams.get('add_fund');

            // Set the deposit amount in the input field
            $('#deposit-amount-input').val(depositAmount);
        }
    });
</script>
